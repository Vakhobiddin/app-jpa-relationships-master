...
public class GroupController {
    ...
    public String editGroup(@PathVariable Integer id, @RequestBody GroupDto groupDto){
        ...
        if (byId.isPresent()){
            ...
            group.setName(groupDto.getName());
            Optional<Faculty> faculcyId = facultyRepository.findById(groupDto.getFacultyId());
            if (faculcyId.isPresent()){
                Faculty faculty = faculcyId.get();
                group.setFaculty(faculty);

            }else {
                return "Faculty id not fount";
            }
